		   ...
		   $STMmm = $dbh->prepare("SELECT * FROM tbl_mtg_attend LEFT JOIN tbl_mtgrmbook ON tbl_mtgrmbook.mtgbook_id = tbl_mtg_attend.booking_id WHERE tbl_mtg_attend.emp_name ='$fname' AND tbl_mtgrmbook.start_date = '$today1'");
			...
			<tr>
			  ...
			  ?>
				...
				<a href="db_cnfrm_mtg.php?mbkid=<?php echo $mm1['mtgbk_id'];?>&u=<?php echo $user;?>" class="btn btn-default">confirm</a>
			  
			  <?php } elseif($stat=='confirmed') {  ?>
			
				<a href="db_mtg_chkin.php?mbkid=<?php echo $mm1['mtgbk_id'];?>&u=<?php echo $user;?>" class="btn btn-default">check-in</a>  
				  
			  <?php } else {} ?>
			  
			  <a href="#mtgview" class="mtgButton btn btn-default" data-mid="<?php echo $mm1['mtgbook_id'];?>" data-toggle="modal" data-target="#mtgview" data-popup='tooltip' title='Add Attendees' data-container='body'>view</a>
			  </td>
